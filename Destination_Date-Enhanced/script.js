    ...
    if(finalArray.length===0){
        ...
        for(let i=1; i<len; i++){
            if(parseInt(finalArray[i].days) < min){
                min = parseInt(finalArray[i].days);
            }
        }
        // console.log(min)
        for(var list in finalArray){
            if(finalArray.hasOwnProperty(list)){
                ...
                if(finalArray[list].days == min){
                    ...
                    reqDate = getDay(dateEntered,min);
                    // reqDate = new Date(dateEntered.getTime() + (min-1)*24*60*60*1000);
                    console.log(finalArray[list].route)
                    bottomContainer.classList.remove("hidden");
                    resultBox.innerHTML = `
                    <p>${finalArray[list].route}</p>
                    <p>Totally ${finalArray[list].days} Working Days</p>
                    <p>Start ${dateEntered.getDate()}${nth(dateEntered.getDate())} ${month[dateEntered.getMonth()]} -> 
                        Arrive on ${reqDate.getDate()}${nth(reqDate.getDate())} ${month[reqDate.getMonth()]}</p>
                    `;
                }
            }
        }
...
function getDay(dateEntered,min){
    console.log(dateEntered,min)
    var Days = [];
    console.log(dateEntered)
    for(let i=0; i<min; i++){
        reqDate = new Date(dateEntered.getTime() + i*24*60*60*1000);
        reqDay = reqDate.getDay();
        Days.push(reqDay)
    }
    if(Days.includes(6)){
        arriveDate = new Date(dateEntered.getTime() + (parseInt(min)+1)*24*60*60*1000);
